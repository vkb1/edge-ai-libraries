  ...
  while (run != 0)
  {
    ...
    if (!shm_blkbuf_empty(handle_r))
    {
      ...
      if (ret) 
      {
        ...
        // }
        waypoint_start = true;
        printf("Joint cmd: %f, %f, %f, %f, %f, %f\n", joint_cmd.positions[0], joint_cmd.positions[1], joint_cmd.positions[2],
                                                      joint_cmd.positions[3], joint_cmd.positions[4], joint_cmd.positions[5]);
      }
    }
    ...
    for (size_t i = 0; i < JOINT_NUM; i ++)
    {
      ...
      if (waypoint_start)
      {
        axis[i]->runCycle(joint_cmd.positions[i] * 180.0 / 3.1415926, 0);
      }
...
int main(int argc, char* argv[])
{
  ...
  for (size_t i = 0; i < JOINT_NUM; i ++)
  {
    configs[i].frequency_ = 1.0 / CYCLE_US * 8000000;
