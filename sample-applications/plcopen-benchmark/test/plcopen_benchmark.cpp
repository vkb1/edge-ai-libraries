                          ...
                          FbMoveRelative** moveRel)
{
  for (size_t i = 0; i < axis_num; i++)
  {
    ...
    moveRel[i] = new FbMoveRelative();
    moveRel[i]->setAxis(axis[i]);
    moveRel[i]->setPosition(-150);
    moveRel[i]->setVelocity(50);
    moveRel[i]->setAcceleration(500);
    moveRel[i]->setDeceleration(500);
    moveRel[i]->setJerk(500);
    moveRel[i]->setBufferMode(mcAborting);
    moveRel[i]->setExecute(mcFALSE);
  }
}
...
void* rt_func(void* arg)
{
  ...
  EcrtServo* my_servo[AXIS_MAX_NUM];
  ...
  /* RTmotion variables */
  init_rtmotion(servo, my_servo, config, axis, fb_power, read_pos, read_vel,
                move_rel);
  ...
  while (run)
  {
    ...
    /* Addressing trace break */
    if (trace_limit && (latency_ns) / 1000 > trace_limit)
    {
      tracemark((char*)"hit latency threshold (%llu > %d)",
                (unsigned long long)fabs(latency_ns) / 1000, trace_limit);
      printf("\nhit latency threshold (%llu > %d)\n",
             (unsigned long long)fabs(latency_ns) / 1000, trace_limit);
    ...
    // uint32_t dc_diff =
    // ecrt_master_sync_monitor_process(master_->master->master);
    clock_gettime(CLOCK_MONOTONIC, &recv_end);

    /* Run RTmotion update */
    if (count_instruction && fd != -1)
    ...
    for (size_t i = 0; i < axis_num; i++)
    {
      if (move_rel[i]->isDone() == mcTRUE)
      {
        ...
        move_rel[i]->setPosition(-move_rel[i]->getPosition());
      }
    }
    clock_gettime(CLOCK_MONOTONIC, &calc_end);
    if (count_instruction && fd != -1)
      stat->instr_count_rt = perf_event_end_count(fd);
    else
      stat->instr_count_rt = 0;
  ...
  /* Clean up */
  clear_rtmotion(servo, my_servo, config, axis, fb_power, read_pos, read_vel,
                 move_rel);
  if (count_instruction && fd != -1)
    close(fd);
...
static void getOptions(int argc, char** argv)
{
  ...
  {
    index =
        getopt_long(argc, argv, "n:i:a:b:m:ct:lvs:orph", long_options, nullptr);
    switch (index)
    {
      case 'n':
        if (eni_file)
