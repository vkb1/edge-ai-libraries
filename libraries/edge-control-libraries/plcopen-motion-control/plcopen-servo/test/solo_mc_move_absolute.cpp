...
static FbReadMotionState fb_read_motion_state;

// log data
static char column_string[] = "FbMoveAbsolute.Execute,\
              FbMoveAbsolute.Done,\
              FbMoveAbsolute.Active,\
              FbMoveAbsolute.Busy,\
              FbMoveAbsolute.Error,\
              FbMoveAbsolute.setPosition,\
              axis_actual_position,\
              FbReadMotionState.ConstantVelocity,\
              FbReadMotionState.Accelerating,\
              FbReadMotionState.Decelerating,\
              FbReadMotionState.DirectionPositive,\
              FbReadMotionState.DirectionNegative";
static char log_file_name[] = "solo_mc_move_absolute_test_results.csv";
...
static fb_move_signal* fb_move_absolute_signal = nullptr;
static fb_read_signal* fb_read_motion_signal   = nullptr;
...
void* my_thread(void* /*arg*/)
{
  ...
  while (run)
  {
    ...
    if (fb_set_position.isEnabled() == mcFALSE && powered_on == mcFALSE &&
        fb_power.getPowerStatus() == mcTRUE)
    {
      fb_set_position.setEnable(mcTRUE);
      printf("Axis is powered on. Set position and execute fb_move.\n");
    }

    fb_move.setExecute(fb_set_position.isEnabled());
    powered_on = fb_move_execute_state = fb_set_position.isEnabled();
    ...
    if (cycle_count % 10000 == 2000)
    {
      ...
      printf("fb_move is execute, Set additive pos:%f,\tCurrent position: "
             "%f,\tCurrent velocity: %f \n",
             fb_move.getPosition(), fb_read_pos.getFloatValue(),
             fb_read_vel.getFloatValue());
    }

    if (fb_move.isError() != mcFALSE)
      printf("FbMoveAbsolute error! Error ID:%d\n", fb_move.getErrorID());

    if (verbose == mcTRUE)
      printf("Current position: %f,\tvelocity: %f,\tcons_vel: "
             "%d,\tacc_ing:%d,\tdir_pos:%d\n",
             fb_read_pos.getFloatValue(), fb_read_vel.getFloatValue(),
             static_cast<bool>(fb_read_motion_state.getConstantVelocity()),
             static_cast<bool>(fb_read_motion_state.getAccelerating()),
             static_cast<bool>(fb_read_motion_state.getDirectionPositive()));
    ...
    if (log_flag == mcTRUE)
    {
      ...
      if (record_cycle_count < record_cycle)
      {
        ...
        fb_move_absolute_signal[record_cycle_count].execute =
            fb_move_execute_state;
        fb_move_absolute_signal[record_cycle_count].done   = fb_move.isDone();
        fb_move_absolute_signal[record_cycle_count].active = fb_move.isActive();
        fb_move_absolute_signal[record_cycle_count].busy   = fb_move.isBusy();
        fb_move_absolute_signal[record_cycle_count].error  = fb_move.isError();
        fb_move_absolute_signal[record_cycle_count].set_value =
            fb_move.getPosition();
        fb_move_absolute_signal[record_cycle_count].actual_value =
            fb_read_pos.getFloatValue();
        fb_read_motion_signal[record_cycle_count].cons_vel =
            fb_read_motion_state.getConstantVelocity();
        fb_read_motion_signal[record_cycle_count].acc_ing =
            fb_read_motion_state.getAccelerating();
        fb_read_motion_signal[record_cycle_count].dec_ing =
            fb_read_motion_state.getDecelerating();
        fb_read_motion_signal[record_cycle_count].dir_pos =
            fb_read_motion_state.getDirectionPositive();
        fb_read_motion_signal[record_cycle_count].dir_neg =
            fb_read_motion_state.getDirectionNegative();
        record_cycle_count++;
      }
    }
    cycle_count++;
...
int main(int argc, char* argv[])
{
  ...
  fb_read_motion_state.setConstantVelocityThreshold(2);

  /* Create csv output */
  if (log_flag == mcTRUE)
  {
    fb_move_absolute_signal =
        (fb_move_signal*)malloc(sizeof(fb_move_signal) * record_cycle);
    memset(fb_move_absolute_signal, 0, sizeof(fb_move_signal) * record_cycle);
    fb_read_motion_signal =
        (fb_read_signal*)malloc(sizeof(fb_read_signal) * record_cycle);
    memset(fb_read_motion_signal, 0, sizeof(fb_read_signal) * record_cycle);
    /* create new csv file */
  ...
  if (log_flag == mcTRUE && record_cycle_count > 0)
  {
    ...
    {
      fprintf(fptr, "%d,%d,%d,%d,%d,%f,%f,%d,%d,%d,%d,%d\n",
              static_cast<bool>(fb_move_absolute_signal[i].execute),
              static_cast<bool>(fb_move_absolute_signal[i].done),
              static_cast<bool>(fb_move_absolute_signal[i].active),
              static_cast<bool>(fb_move_absolute_signal[i].busy),
              static_cast<bool>(fb_move_absolute_signal[i].error),
              fb_move_absolute_signal[i].set_value,
              fb_move_absolute_signal[i].actual_value,
              static_cast<bool>(fb_read_motion_signal[i].cons_vel),
              static_cast<bool>(fb_read_motion_signal[i].acc_ing),
              static_cast<bool>(fb_read_motion_signal[i].dec_ing),
              static_cast<bool>(fb_read_motion_signal[i].dir_pos),
              static_cast<bool>(fb_read_motion_signal[i].dir_neg));
    }
    fclose(fptr);
  }
  ...
  delete my_servo;
  delete axis;
  free(fb_move_absolute_signal);
  free(fb_read_motion_signal);
