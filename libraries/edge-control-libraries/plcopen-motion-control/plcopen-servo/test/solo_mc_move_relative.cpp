...
static mcLINT ratio    = PER_CIRCLE_ENCODER;
...
// log data
static char column_string[] = "FbMoveRelative.Execute,\
              FbMoveRelative.Done,\
              FbMoveRelative.Active,\
              FbMoveRelative.Busy,\
              FbMoveRelative.Error,\
              FbMoveRelative.setPosition,\
              axis_actual_position";
static char log_file_name[] = "solo_mc_move_relative_test_results.csv";
...
static fb_move_signal* fb_move_relative_signal = nullptr;
...
void* my_thread(void* /*arg*/)
{
  ...
  while (run)
  {
    ...
    if (cycle_count % 10000 == 2000)
      printf("fb_move is execute, Set relative pos:%f,\tCurrent position: "
    ...
    if (log_flag == mcTRUE)
    {
      ...
      if (record_cycle_count < record_cycle)
      {
        ...
        fb_move_relative_signal[record_cycle_count].execute =
            fb_move_execute_state;
        fb_move_relative_signal[record_cycle_count].done   = fb_move.isDone();
        fb_move_relative_signal[record_cycle_count].active = fb_move.isActive();
        fb_move_relative_signal[record_cycle_count].busy   = fb_move.isBusy();
        fb_move_relative_signal[record_cycle_count].error  = fb_move.isError();
        fb_move_relative_signal[record_cycle_count].set_value =
            fb_move.getPosition();
        fb_move_relative_signal[record_cycle_count].actual_value =
...
int main(int argc, char* argv[])
{
  ...
  if (log_flag == mcTRUE)
  {
    fb_move_relative_signal =
        (fb_move_signal*)malloc(sizeof(fb_move_signal) * record_cycle);
    memset(fb_move_relative_signal, 0, sizeof(fb_move_signal) * record_cycle);
    /* create new csv file */
  ...
  if (log_flag == mcTRUE && record_cycle_count > 0)
  {
    ...
    for (size_t i = 0; i < record_cycle_count; i++)
    {
      fprintf(fptr, "%d,%d,%d,%d,%d,%f,%f\n",
              static_cast<bool>(fb_move_relative_signal[i].execute),
              static_cast<bool>(fb_move_relative_signal[i].done),
              static_cast<bool>(fb_move_relative_signal[i].active),
              static_cast<bool>(fb_move_relative_signal[i].busy),
              static_cast<bool>(fb_move_relative_signal[i].error),
              fb_move_relative_signal[i].set_value,
              fb_move_relative_signal[i].actual_value);
