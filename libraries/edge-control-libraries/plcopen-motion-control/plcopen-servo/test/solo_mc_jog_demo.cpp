...
// RTmotion related
#define SET_JOG_VEL 50
#define SET_JOG_ACC 50
#define SET_JOG_DCC 50
#define SET_JOG_JERK 500
static mcBOOL jog_forward_sig  = mcFALSE;
static mcBOOL jog_backward_sig = mcFALSE;
...
/* FbJog */
static bool* jog_fb_forward_ptr      = nullptr;
static bool* jog_fb_backward_ptr     = nullptr;
static float* jog_fb_vel_ptr         = nullptr;
static float* jog_fb_acc_ptr         = nullptr;
static float* jog_fb_dcc_ptr         = nullptr;
static float* jog_fb_jerk_ptr        = nullptr;
static bool* jog_fb_done_ptr         = nullptr;
static bool* jog_fb_busy_ptr         = nullptr;
static bool* jog_fb_abort_ptr        = nullptr;
static bool* jog_fb_error_ptr        = nullptr;
static uint16_t* jog_fb_error_id_ptr = nullptr;
...
static JOG_TEST s_jog_test_func  = MC_JOG_IDLE;
...
void* my_thread(void* /*arg*/)
{
  ...
  while (run)
  {
    ...
    switch (s_jog_test_func)
    {
      case MC_JOG_IDLE: {
        if (powered_on == mcTRUE)
        {
          jog_forward_sig  = mcTRUE;
          jog_backward_sig = mcFALSE;
          fb_jog.setExecute(mcTRUE);
          fb_jog.setJogForwardSignal(jog_forward_sig);
          fb_jog.setJogBackwardSignal(jog_backward_sig);
          s_jog_test_func = MC_JOG_FORWARD;
          printf("Axis is powered on. Continue to execute fb_jog forward.\n");
        }
      }
      break;
      case MC_JOG_FORWARD: {
        if (s_func_test_count++ > 3000)
        {
          jog_forward_sig  = mcFALSE;
          jog_backward_sig = mcTRUE;
          fb_jog.setJogForwardSignal(jog_forward_sig);
          fb_jog.setJogBackwardSignal(jog_backward_sig);
          s_func_test_count = 0;
          s_jog_test_func   = MC_JOG_BACKWARD;
          printf("Next to execute fb_jog backward.\n");
        }
      }
      break;
      case MC_JOG_BACKWARD: {
        if (s_func_test_count++ > 3000)
        {
          jog_forward_sig  = mcTRUE;
          jog_backward_sig = mcTRUE;
          ...
          fb_jog.setJogBackwardSignal(jog_backward_sig);
          s_func_test_count = 0;
          s_jog_test_func   = MC_JOG_HALT;
          printf("Next to execute fb_jog halt.\n");
        }
      }
      break;
      case MC_JOG_HALT: {
        ...
        {
          s_func_test_count = 0;
          s_jog_test_func   = MC_JOG_IDLE;
          printf("Next to execute fb_jog Idle.\n");
        }
        else if (s_func_test_count == 1500)
        {
          jog_forward_sig  = mcFALSE;
          jog_backward_sig = mcFALSE;
          fb_jog.setJogForwardSignal(jog_forward_sig);
    ...
    if (log_flag)
    {
      ...
      if (recorded_cycle < running_cycle_10min)
      {
        ...
        /* FbJog */
        jog_fb_forward_ptr[recorded_cycle]  = (bool)jog_forward_sig;
        jog_fb_backward_ptr[recorded_cycle] = (bool)jog_backward_sig;
        jog_fb_vel_ptr[recorded_cycle]      = SET_JOG_VEL;
        jog_fb_acc_ptr[recorded_cycle]      = SET_JOG_ACC;
        jog_fb_dcc_ptr[recorded_cycle]      = SET_JOG_DCC;
        jog_fb_jerk_ptr[recorded_cycle]     = SET_JOG_JERK;
        jog_fb_done_ptr[recorded_cycle]     = (bool)fb_jog.isDone();
        jog_fb_busy_ptr[recorded_cycle]     = (bool)fb_jog.isBusy();
        jog_fb_abort_ptr[recorded_cycle]    = (bool)fb_jog.isAborted();
        jog_fb_error_ptr[recorded_cycle]    = (bool)fb_jog.isError();
        jog_fb_error_id_ptr[recorded_cycle] = fb_jog.getErrorID();
...
static void save_log(unsigned int len, char* log_file)
{
  ...
  for (size_t i = 0; i < len; ++i)
  {
    ...
    /* FbJog */
    fprintf(fptr, "%d,", jog_fb_forward_ptr[i]);
    fprintf(fptr, "%d,", jog_fb_backward_ptr[i]);
    fprintf(fptr, "%f,", jog_fb_vel_ptr[i]);
    fprintf(fptr, "%f,", jog_fb_acc_ptr[i]);
    fprintf(fptr, "%f,", jog_fb_dcc_ptr[i]);
    fprintf(fptr, "%f,", jog_fb_jerk_ptr[i]);
    fprintf(fptr, "%d,", jog_fb_done_ptr[i]);
    fprintf(fptr, "%d,", jog_fb_busy_ptr[i]);
    fprintf(fptr, "%d,", jog_fb_abort_ptr[i]);
    fprintf(fptr, "%d,", jog_fb_error_ptr[i]);
    fprintf(fptr, "%d\n", jog_fb_error_id_ptr[i]);
  }
...
static void init_stat()
{
  ...
  /* FbJog */
  jog_fb_forward_ptr = (bool*)malloc(sizeof(bool) * uint(running_cycle_10min));
  memset(jog_fb_forward_ptr, 0, sizeof(bool) * uint(running_cycle_10min));
  jog_fb_backward_ptr = (bool*)malloc(sizeof(bool) * uint(running_cycle_10min));
  memset(jog_fb_backward_ptr, 0, sizeof(bool) * uint(running_cycle_10min));
  jog_fb_vel_ptr = (float*)malloc(sizeof(float) * uint(running_cycle_10min));
  memset(jog_fb_vel_ptr, 0, sizeof(float) * uint(running_cycle_10min));
  jog_fb_acc_ptr = (float*)malloc(sizeof(float) * uint(running_cycle_10min));
  memset(jog_fb_acc_ptr, 0, sizeof(float) * uint(running_cycle_10min));
  jog_fb_dcc_ptr = (float*)malloc(sizeof(float) * uint(running_cycle_10min));
  memset(jog_fb_dcc_ptr, 0, sizeof(float) * uint(running_cycle_10min));
  jog_fb_jerk_ptr = (float*)malloc(sizeof(float) * uint(running_cycle_10min));
  memset(jog_fb_jerk_ptr, 0, sizeof(float) * uint(running_cycle_10min));
  jog_fb_done_ptr = (bool*)malloc(sizeof(bool) * uint(running_cycle_10min));
  memset(jog_fb_done_ptr, 0, sizeof(bool) * uint(running_cycle_10min));
  jog_fb_busy_ptr = (bool*)malloc(sizeof(bool) * uint(running_cycle_10min));
  memset(jog_fb_busy_ptr, 0, sizeof(bool) * uint(running_cycle_10min));
  jog_fb_abort_ptr = (bool*)malloc(sizeof(bool) * uint(running_cycle_10min));
  memset(jog_fb_abort_ptr, 0, sizeof(bool) * uint(running_cycle_10min));
  jog_fb_error_ptr = (bool*)malloc(sizeof(bool) * uint(running_cycle_10min));
  memset(jog_fb_error_ptr, 0, sizeof(bool) * uint(running_cycle_10min));
  jog_fb_error_id_ptr =
      (uint16_t*)malloc(sizeof(uint16_t) * uint(running_cycle_10min));
  memset(jog_fb_error_id_ptr, 0, sizeof(uint16_t) * uint(running_cycle_10min));
}
...
int main(int argc, char* argv[])
{
  ...
  fb_jog.setAxis(axis);
  fb_jog.setVelocity(SET_JOG_VEL);
  fb_jog.setAcceleration(SET_JOG_ACC);
  fb_jog.setDeceleration(SET_JOG_DCC);
  fb_jog.setJerk(SET_JOG_JERK);
  fb_jog.setBufferMode(mcAborting);
  fb_jog.setExecute(mcFALSE);
  ...
  free(act_toq_ptr);
  /* FbJog */
  free(jog_fb_forward_ptr);
  free(jog_fb_backward_ptr);
  free(jog_fb_vel_ptr);
  free(jog_fb_acc_ptr);
  free(jog_fb_dcc_ptr);
  free(jog_fb_jerk_ptr);
  free(jog_fb_done_ptr);
  free(jog_fb_busy_ptr);
  free(jog_fb_abort_ptr);
  free(jog_fb_error_ptr);
  free(jog_fb_error_id_ptr);
