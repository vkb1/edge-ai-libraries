...
static mcULINT ratio      = 1 << 23;
static mcINT numerator    = 1;
...
static FbGearInPos fb_gear_in_pos;
...
// log data
static char column_string[] = "fb_gear_in_pos.Execute,\
              fb_gear_in_pos.InSync,\
              fb_gear_in_pos.Busy,\
              fb_gear_in_pos.Active,\
              fb_gear_in_pos.Error,\
              fb_gear_out.Execute,\
              fb_gear_out.Done,\
              fb_gear_out.Busy,\
              fb_gear_out.Error,\
              ...
              slave_pos,\
              slave_axis_state";
static char log_file_name[] = "duo_rtmotion_gear_in_pos_demo_test_results.csv";
...
static fb_gear_signal* fb_gear_in_pos_signal = nullptr;
static fb_gear_signal* fb_gear_out_signal    = nullptr;
static mcREAL* master_vel_ptr                = nullptr;
static mcREAL* master_pos_ptr                = nullptr;
static mcREAL* slave_vel_ptr                 = nullptr;
static mcREAL* slave_pos_ptr                 = nullptr;
static mcUSINT* slave_axis_state_ptr         = nullptr;
mcBOOL fb_gear_in_pos_execute_state;
mcBOOL fb_gear_out_execute_state;
mcULINT record_cycle       = 35000;
mcULINT record_cycle_count = 0;
mcBOOL in_error            = mcFALSE;
...
void* my_thread(void* /*arg*/)
{
  ...
  while (run)
  {
    ...
    if (fb_power_1.getPowerStatus() == mcTRUE &&
        fb_power_2.getPowerStatus() == mcTRUE && cycle_count > 1000)
    {
      fb_move_vel.setExecute(mcTRUE);
      if (cycle_count < 5000)
      {
        fb_gear_in_pos.setExecute(mcTRUE);
        fb_gear_in_pos_execute_state = mcTRUE;
      }
    }
    ...
    /* falling edge test of MC_GearInPos */
    if (cycle_count == 12000)
    {
      fb_gear_out.setExecute(mcTRUE);
      fb_gear_in_pos.setExecute(mcFALSE);
      fb_gear_in_pos_execute_state = mcFALSE;
    }

    /* reexectue MC_GearInPos */
    if (cycle_count == 13000)
    {
      fb_gear_out.setExecute(mcFALSE);
      fb_gear_in_pos.setMasterStartDistance(2);
      fb_gear_in_pos.setMasterSyncPosition(12);
      fb_gear_in_pos.setSlaveSyncPosition(7);
      fb_gear_in_pos.setVelocity(50);
      fb_gear_in_pos.setRatioNumerator(2);
      fb_gear_in_pos.setExecute(mcTRUE);
      fb_gear_in_pos_execute_state = mcTRUE;
    }
    ...
    if (verbose == mcTRUE)
    {
      printf("Current Master position: %f Current Slave position: %f \n"
             "Current Master velocity: %f Current Slave velocity: %f \n"
             "Current Slave state: %d\n",
             fb_read_pos_1.getFloatValue(), fb_read_pos_2.getFloatValue(),
             fb_read_vel_1.getFloatValue(), fb_read_vel_2.getFloatValue(),
             axis_2->getAxisState());
    }

    if (fb_gear_in_pos.isError() == mcTRUE && in_error == mcFALSE)
    {
      printf("Cycle: %ld: FbGearInPos error! Error ID:%d, master pos: %f\n",
             cycle_count, fb_gear_in_pos.getErrorID(),
             fb_read_pos_1.getFloatValue());
      in_error = mcTRUE;
    }
    if (fb_gear_out.isError() == mcTRUE && in_error == mcFALSE)
    {
    ...
    if (log_flag == mcTRUE)
    {
      ...
      if (cycle_count < record_cycle)
      {
        ...
        fb_gear_in_pos_signal[record_cycle_count].execute =
            fb_gear_in_pos_execute_state;
        fb_gear_in_pos_signal[record_cycle_count].in_sync =
            fb_gear_in_pos.getInSync();
        fb_gear_in_pos_signal[record_cycle_count].active =
            fb_gear_in_pos.isActive();
        fb_gear_in_pos_signal[record_cycle_count].busy =
            fb_gear_in_pos.isBusy();
        fb_gear_in_pos_signal[record_cycle_count].error =
            fb_gear_in_pos.isError();
        ...
        master_pos_ptr[record_cycle_count] = fb_read_pos_1.getFloatValue();
        slave_vel_ptr[record_cycle_count]  = fb_read_vel_2.getFloatValue();
        slave_pos_ptr[record_cycle_count]  = fb_read_pos_2.getFloatValue();
...
int main(int argc, char* argv[])
{
  ...
  fb_move_vel.setAcceleration(5);
  fb_move_vel.setDeceleration(5);
  fb_move_vel.setJerk(500);
  fb_move_vel.setBufferMode(mcAborting);
  fb_move_vel.setExecute(mcFALSE);

  /* initialize MC_GearIn */
  fb_gear_in_pos.setMaster(axis_1);
  fb_gear_in_pos.setSlave(axis_2);
  fb_gear_in_pos.setMasterStartDistance(2);
  fb_gear_in_pos.setMasterSyncPosition(6);
  fb_gear_in_pos.setRatioNumerator(numerator);
  fb_gear_in_pos.setRatioDenominator(denominator);
  fb_gear_in_pos.setSlaveSyncPosition(2);
  fb_gear_in_pos.setVelocity(50);
  fb_gear_in_pos.setAcceleration(500);
  fb_gear_in_pos.setDeceleration(500);
  fb_gear_in_pos.setJerk(5000);
  ...
  if (log_flag == mcTRUE)
  {
    fb_gear_in_pos_signal =
        (fb_gear_signal*)malloc(sizeof(fb_gear_signal) * record_cycle);
    memset(fb_gear_in_pos_signal, 0, sizeof(fb_gear_signal) * record_cycle);
    fb_gear_out_signal =
        (fb_gear_signal*)malloc(sizeof(fb_gear_signal) * record_cycle);
    ...
    master_pos_ptr = (mcREAL*)malloc(sizeof(mcREAL) * record_cycle);
    memset(master_pos_ptr, 0, sizeof(mcREAL) * record_cycle);
    slave_vel_ptr = (mcREAL*)malloc(sizeof(mcREAL) * record_cycle);
    memset(slave_vel_ptr, 0, sizeof(mcREAL) * record_cycle);
    slave_pos_ptr = (mcREAL*)malloc(sizeof(mcREAL) * record_cycle);
    memset(slave_pos_ptr, 0, sizeof(mcREAL) * record_cycle);
  ...
  if (log_flag == mcTRUE && record_cycle_count > 0)
  {
    ...
    for (size_t i = 0; i < record_cycle_count; i++)
    {
      fprintf(fptr, "%d,%d,%d,%d,%d,",
              static_cast<bool>(fb_gear_in_pos_signal[i].execute),
              static_cast<bool>(fb_gear_in_pos_signal[i].in_sync),
              static_cast<bool>(fb_gear_in_pos_signal[i].active),
              static_cast<bool>(fb_gear_in_pos_signal[i].busy),
              static_cast<bool>(fb_gear_in_pos_signal[i].error));
      ...
      fprintf(fptr, "%f,%f,%f,%f,%d\n", master_vel_ptr[i], master_pos_ptr[i],
              slave_vel_ptr[i], slave_pos_ptr[i], slave_axis_state_ptr[i]);
    }
    fclose(fptr);
  }
