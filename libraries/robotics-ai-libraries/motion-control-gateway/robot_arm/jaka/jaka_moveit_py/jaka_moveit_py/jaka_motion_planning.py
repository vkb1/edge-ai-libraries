...
class LLMTaskExecutor(object):

    def __init__(self):
        self.logger = get_logger("jaka_moveit_py")
        self.logger.info("MoveItPy setup Done")

        # instantiate MoveItPy instance and get planning component
        self.jaka_py = MoveItPy(node_name="moveit_py")
        self.jaka_arm = self.jaka_py.get_planning_component("jaka_arm")
        self.logger.info("MoveItPy instance created")

        file_path=get_package_share_directory("jaka_description") + "/urdf/jaka_system_can.urdf.xacro"
    ...
    def write_coils(self, addr, value):
        try:
            ...
            return False
        if ww.isError():
            print(f"Received Modbus library error({ww})")
            self.modbus_client.close()
            return False
        if isinstance(ww, ExceptionResponse):
            print(f"Received Modbus library exception ({ww})")
    ...
    #    jaka_arm.set_start_state(configuration_name="home")
        self.jaka_arm.set_start_state_to_current_state()
        self.jaka_arm.set_goal_state(configuration_name="ready")

        # plan to goal
        self.logger.info("Planning trajectory to: ready")
        plan_result = self.jaka_arm.plan()

        # execute the plan
        if plan_result:
            self.logger.info("Executing plan")
            robot_trajectory = plan_result.trajectory
            self.jaka_py.execute(robot_trajectory, controllers=[])
    ...
    def move(self, target_pose):
        ...
        #   target_pose
        ###################################################################
        q1 = np.array([-0.369, 0.171734, -1.387, 0.009132, -1.57275, -1.9929])
        ...
        robot_model = self.jaka_py.get_robot_model()
        ...
        self.jaka_arm.set_start_state_to_current_state()
        # self.jaka_arm.set_goal_state(pose_stamped_msg=target_pose, pose_link="tool0")
        self.jaka_arm.set_goal_state(robot_state=robot_state)
...
obj_info = {}
logger = get_logger("jaka_moveit_py")
    ...
    for obj in obj_info.values():
        if obj_name == obj['Name']:
            f_camera2base = PyKDL.Frame(
                    PyKDL.Rotation.Quaternion(0.008037649900302495, 0.7123189243673227, -0.05508222270698046, 0.699644977766523),
                    PyKDL.Vector(-0.5851423133597079, -0.04454649215058375, 0.7670624463956265))
