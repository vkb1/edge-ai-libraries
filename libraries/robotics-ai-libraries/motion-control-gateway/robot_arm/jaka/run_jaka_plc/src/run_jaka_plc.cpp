...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("run_jaka_plc");

/* JAKA robot joint name and initial joint values */
...
public:
  ...
  void timerCB(void)
  {
    ...
    // RCLCPP_INFO(LOGGER, "Publish state: %f, %f, %f, %f, %f, %f\n", joint_msg_->position[0], joint_msg_->position[1],
  ...
  void receiveJointStateFromRT(std::vector<double>& joint_state)
  {
    ...
    if (!shm_blkbuf_empty(handle_s))
    {
      ...
      if (ret)
      {
        ...
        // RCLCPP_INFO(LOGGER, "Receive state: %f, %f, %f, %f, %f, %f\n", j_state.joint_pos[0], j_state.joint_pos[1],
        //                                                                j_state.joint_pos[2], j_state.joint_pos[3],
        //                                                                j_state.joint_pos[4], j_state.joint_pos[5]);
...
int main(int argc, char** argv)
{
  ...
  rclcpp::Node::SharedPtr node = rclcpp::Node::make_shared("run_jaka_plc", "", node_options);
